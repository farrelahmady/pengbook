...
**Status**: Tahap 2 (F1–F8) Selesai — `GET /tree`, `postingCount`, create akun + codegen, rename Coa→Account, edit rename-only, search tree frontend, error state + retry, controlled expand/collapse + aria, i18n badge/subtitle + Export Excel daftar akun (`GET /export`, tombol download di toolbar list)
